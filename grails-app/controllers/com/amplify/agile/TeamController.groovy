...
class TeamController {
    ...
    def update(Team teamInstance) {
        // crazy town
        if (teamInstance.users.size() > 1) {
            def oldUsersArray = Eval.me(params.oldUsers.toString())
            oldUsersArray.each { u ->
                def userToRemove = null
                    teamInstance.users.each {
                    if (it.id == u) {
                        userToRemove = teamInstance.users.find { it.id == u }
                    }
                }
                if (userToRemove)
                {
                    teamInstance.users.remove(userToRemove)
                }
            }
        }
